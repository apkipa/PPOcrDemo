        ...
        RotatedRect getMinimumBoundingRectangleImpl(std::span<T const> hull) {
            ...
            for (size_t i = 0; i < n; i++) {
                ...
                if (area < minArea) {
                    ...
                    // 旋转角度（以x轴为基准，逆时针，弧度）
                    // Need to limit rotation angle to [-π/4, π/4], and / or swap the width and height accordingly
                    if (ux < 0) {
...
SoftwareBitmap PPOcr::ExtractQuadFromBitmap(SoftwareBitmap const& src, Quad const& quad) {
    ...
    // getMinimumBoundingRectangle tends to return a rectangle with rotation within [-π/4, π/4],
